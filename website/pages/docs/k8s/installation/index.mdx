...
We recommend setting the `global.name` value to `"consul"` so resources
don't have the release name prefixed:
...
With this setting, the server pods will be named `consul-server-0` instead of
